...
#rank 5 players above and below for part h
def close_scores(pscore):
    username = input("Please enter your username: ")
    #find position in array, linear search by username
    pos=-1
    ...
    if pos==-1:
        ...
        toplist = []
        #count towards 0 for 5 users above in leaderboard
        curr=pos-1
        for i in range(0, 5):
            if curr<0:
                break
            toplist.append(pscore[curr])
            #break if reached top of leaderboard
            curr-=1
        #reverse toplist to get descending order
        toplist=toplist[::-1]
        #count towards bottom of leaderboard for 5 users
        bottomlist=[]
        curr=pos+1
        for i in range(0, 5):
            if curr>=len(pscore):
...
def menu():
    ...
    print("[2] Find player's score")
    print("[3] Find all ties in player scores")
    print("[4] Find player's near you")
    print("[5] Display Leaderboard")
    print("[0] Exit")
...
pscore = name_sort(pscore)
print("Random leaderboard generated.")
while True:
    menu()
    select = input()
    if select=="0":
        ...
        username = input("Please enter player username: ")
        score = input("Please enter player score: ")
        pscore = insert_player(username, score, pscore)
    elif select=="2":
        search_player(pscore)
    elif select=="3":
        same_score(pscore)
    elif select=="4":
        close_scores(pscore)
